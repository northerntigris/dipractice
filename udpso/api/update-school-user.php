...
try {
    ...
    $updStmt = $pdo->prepare("UPDATE users SET full_name = :full_name, email = :email WHERE id = :user_id");
    $updStmt->execute([
        ':full_name' => $fullName,
        ':email' => $email,
        ':user_id' => $userId
    ...
    echo json_encode(['success' => false, 'error' => 'Ошибка обновления пользователя'], JSON_UNESCAPED_UNICODE);
}
